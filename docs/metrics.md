...
| `mailtool.watcher.new_mail.messages` | Counter | {message} | `account.id`, `mailbox` | Count of new messages observed by the watcher — incremented by the actual `count - previousCount` delta on each `newMail` event, not just 1 per event. | `telemetry/watcherMetrics.ts` |
